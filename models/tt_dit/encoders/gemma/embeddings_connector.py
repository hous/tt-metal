...
class EmbeddingsConnector(Module):
    ...
    def __init__(
        ...
        *,
        ...
        # RoPE cos/sin depend only on seq_len (dim/heads/theta are fixed), so build once per length
        # and reuse — the grids were being recomputed on host and re-uploaded on every encode.
    ...
    def forward(self, features: ttnn.Tensor, attn_mask: torch.Tensor, *, trans_mat: ttnn.Tensor) -> torch.Tensor:
        ...
        # Connector RoPE on device (cached per seq_len). Checkpoint is rope_type=SPLIT, but the
        # block's Q/K (and q_norm/k_norm) weights were permuted at load (SPLIT→INTERLEAVED), so the
        # on-device rotary_embedding_llama interleaved kernel is equivalent.
