...
TEST(RealtimeProfilerStress, CallbackReactivity) {
    ...
    // Back-to-back floor: one replay fires every burst op with no host pacing, so they come out at the
    // device's record rate. The first burst dispatch also flushes the last paced op.
    mesh_device->quiesce_devices();
    mesh_device->replay_mesh_trace(cq.id(), burst_trace_id, true);

    // Each op's record is only emitted on the next dispatch (device-side flush), so trailing dispatches
    // flush the tail (the last paced op and the last burst op). They carry kStressRuntimeId, so the matcher
    // ignores them.
    ...
    // Back-to-back floor: op-to-op from the records' device timestamps (not host delivery, which arrives in
    // batches), so it reflects the rate the device actually emitted records -- the spacing the Tracy Program
    // zones show. frequency is device cycles per ns.
    ...
    if (burst_matched >= 8 && freq > 0.0) {
        ...
        // wake-from-idle latency) and the last op (its record only flushes on the trailing dispatch, so it is
