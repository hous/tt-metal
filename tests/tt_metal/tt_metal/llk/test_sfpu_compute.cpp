...
struct SfpuConfig {
    ...
    bool unpack_to_dest_en = false;  // explicit unpack-to-dest WITHOUT forcing fp32 (e.g. 16-bit to DEST)
...
void run_quasar_sfpu_unpack_to_dest_16b(
    ...
    unit_tests::compute::sfpu::SfpuConfig cfg{
        ...
        .approx_mode = false,
        .dst_full_sync_en = dst_full_sync_en,
        .unpack_to_dest_en = true,  // 16-bit operand routed to DEST via the explicit flag (fp32_dest_acc_en stays false)
    };
    log_info(
...
TEST_F(QuasarMeshDeviceSingleCardFixture, QuasarSfpuUnpackToDest16b) {
    // 16-bit operand explicitly routed to DEST via unpack_to_dest_en — impossible before the
