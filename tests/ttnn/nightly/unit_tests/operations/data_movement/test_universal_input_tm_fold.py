...
def _assert_output_layout_contract(result, input_mem_config):
    """Universal-IO contract: output TensorMemoryLayout matches input's (no silent demotion)."""
    ...
    if expected_layout in sharded_layouts:
        assert result.memory_config().shard_spec is not None, "Sharded output missing shard_spec"
