...
@pytest.mark.parametrize("use_all_gather", [pytest.param(True, marks=pytest.mark.skip(reason=LEGACY_CCL_SKIP)), False])
...
def test_multi_device_single_trace(mesh_device, shape, use_all_gather, enable_multi_cq):
...
@pytest.mark.parametrize("mesh_device", [(1, 8)], indirect=True)
def test_multi_device_multi_trace(mesh_device, shape, use_all_gather, enable_multi_cq):
    torch.manual_seed(0)
    if mesh_device.get_num_devices() <= 1:
        pytest.skip("This test requires multiple devices")
