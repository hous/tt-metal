...
struct ComputeHardwareConfig {
    ...
    // (Quasar only) Explicitly route this kernel's unpacked operands into the math DEST register
    // (and run the unpack→math→pack semaphore handshake), independent of operand data format.
