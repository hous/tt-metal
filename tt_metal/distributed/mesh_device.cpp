...
bool MeshDeviceImpl::close_impl(MeshDevice* pimpl_wrapper) {
    ...
    if (is_initialized()) {
        ...
        const bool has_slow_dispatch_queue =
            std::any_of(mesh_command_queues_.begin(), mesh_command_queues_.end(), [](const auto& command_queue) {
                return dynamic_cast<SDMeshCommandQueue*>(command_queue.get()) != nullptr;
            });
        if (has_slow_dispatch_queue) {
            for (auto* device : view_->get_devices()) {
